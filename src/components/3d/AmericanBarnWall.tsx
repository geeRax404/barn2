...
interface AmericanBarnWallProps {
...
const AmericanBarnWall: React.FC<AmericanBarnWallProps> = ({ 
  position, 
  width, 
  height, 
  color, 
  ...
  const wallMaterial = useMemo(() => {
    ...
    if (ctx) {
      ...
      console.log(`🏗️ CREATING AMERICAN BARN WALL ${wallProfile.toUpperCase()} PROFILE: ${profileType} for ${wallPosition} wall`);
      ...
      console.log(`✅ AMERICAN BARN WALL ${wallProfile.toUpperCase()} PROFILE TEXTURE CREATED for ${wallPosition}`);
    }
  ...
  const wallGeometry = useMemo(() => {
    ...
    console.log(`🏗️ Creating AMERICAN BARN wall geometry for ${wallPosition} with ${windowFeatures.length} window cutouts`);

    // Calculate roof height for American barn roof
  ...
  const beamSegments = useMemo(() => {
    console.log(`\n🏗️  AMERICAN BARN WALL STRUCTURAL BEAM GENERATION for ${wallPosition} wall (${width}x${height})`);
  ...
  const horizontalBeamSegments = useMemo(() => {
    console.log(`\n🏗️  AMERICAN BARN WALL HORIZONTAL STRUCTURAL BEAM GENERATION for ${wallPosition} wall`);
  ...
  const createInteriorBeam = (segment: BeamSegment, segmentIndex: number) => {
    ...
    const key = `american-barn-interior-beam-${wallPosition}-${segment.x}-${segment.bottomY}-${segment.topY}-${segmentIndex}`;
  ...
  const createInteriorHorizontalBeam = (segment: BeamSegment, segmentIndex: number) => {
    ...
    const key = `american-barn-interior-h-beam-${wallPosition}-${segment.x}-${segment.bottomY}-${segment.topY}-${segment.width}-${segmentIndex}`;
...
export default AmericanBarnWall;