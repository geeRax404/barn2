...
const GableWall: React.FC<GableWallProps> = ({ 
  position, 
  width, 
  height, 
  color, 
  ...
  const wallMaterial = useMemo(() => {
    ...
    if (ctx) {
      ...
      console.log(`🏗️ CREATING GABLE WALL ${wallProfile.toUpperCase()} PROFILE: ${profileType}`);
      ...
      console.log(`✅ GABLE WALL ${wallProfile.toUpperCase()} PROFILE TEXTURE CREATED`);
    }
  ...
  const wallGeometry = useMemo(() => {
    ...
    console.log(`🏗️ Creating GABLE wall geometry for ${wallPosition} with ${windowFeatures.length} window cutouts`);
  ...
  // Generate structural beams that split around ALL features (including doors)
  const beamSegments = useMemo(() => {
    console.log(`\n🏗️  GABLE WALL STRUCTURAL BEAM GENERATION for ${wallPosition} wall (${width}x${height})`);
  ...
  // Generate horizontal beams that split around ALL features (including doors)
  const horizontalBeamSegments = useMemo(() => {
    console.log(`\n🏗️  GABLE WALL HORIZONTAL STRUCTURAL BEAM GENERATION for ${wallPosition} wall`);
  ...
  // Create interior beam segments with architectural integrity
  const createInteriorBeam = (segment: BeamSegment, segmentIndex: number) => {
    ...
    console.log(`🔧 Beam ${segmentIndex} on ${wallPosition} wall positioned at Z = ${zOffset} (GUARANTEED INTERIOR)`);
    ...
    const key = `gable-interior-beam-${wallPosition}-${segment.x}-${segment.bottomY}-${segment.topY}-${segmentIndex}`;
  ...
  // Create horizontal beam segments
  const createInteriorHorizontalBeam = (segment: BeamSegment, segmentIndex: number) => {
    ...
    const zOffset = getInteriorZOffset(wallPosition);
    console.log(`🔧 Horizontal beam ${segmentIndex} on ${wallPosition} wall positioned at Z = ${zOffset} (GUARANTEED INTERIOR)`);
    ...
    const key = `gable-interior-h-beam-${wallPosition}-${segment.x}-${segment.bottomY}-${segment.topY}-${segment.width}-${segmentIndex}`;
