...
interface AmericanBarnRoofProps {
  width: number;
  length: number;
  height: number;
  pitch: number;
...
const AmericanBarnRoof: React.FC<AmericanBarnRoofProps> = ({ 
  width, 
  length, 
  height, 
  pitch, 
  ...
  const roofHeight = useMemo(() => {
    // For American barn clerestory roof, calculate the rise of the monitor section
    return (width * 0.15) * (pitch / 12); // 15% of width for the monitor rise
  }, [width, pitch]);

  // Create roof materials and geometries for American barn clerestory roof
  const { leftRoofGeometry, rightRoofGeometry, monitorRoofLeftGeometry, monitorRoofRightGeometry, roofMaterial } = useMemo(() => {
    ...
    const createAmericanBarnRoofTexture = () => {
      ...
      if (ctx) {
        ...
        console.log(`🏗️ CREATING AMERICAN BARN ROOF ${wallProfile.toUpperCase()} PROFILE: ${profileType}`);
        ...
        console.log(`✅ AMERICAN BARN ROOF ${wallProfile.toUpperCase()} PROFILE TEXTURE CREATED`);
      }
      ...
      // Texture scaling for American barn roof
      let scaleX = width / 4;
      let scaleY = length / 3;
    ...
    const roofTexture = createAmericanBarnRoofTexture();
    ...
    // Create American barn roof geometries - main roof and monitor section
    console.log(`🏗️ Creating AMERICAN BARN ROOF: ${width}ft × ${length}ft, ${pitch}:12 pitch`);
  ...
  const createSkylight = (skylight: Skylight, index: number) => {
    ...
    return (
      <mesh
        key={`american-barn-skylight-${index}`}
  ...
  return (
    <group position={[0, height, 0]}>
      ...
      {/* Skylights for American barn roof */}
...
export default AmericanBarnRoof;